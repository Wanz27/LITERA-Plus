...
interface Props {
  initial: AuthUser
  onClose: () => void
  onSubmit: (payload: { full_name: string; email: string }) => Promise<void>
}

export default function EditProfileModal({ initial, onClose, onSubmit }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      await onSubmit({ full_name: fullName.trim(), email: email.trim() })
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-xl sm:p-6">
        <div className="mb-6 flex items-center justify-between">
          <h3 className="text-lg font-bold text-slate-900">Ubah Profil</h3>
